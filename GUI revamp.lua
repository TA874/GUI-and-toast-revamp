...
local function autoGetColor()
	local cr,cg,cb=giGetColor()
	local i=(settings.r/3)+(settings.g/3)+(settings.b/3)
	if i<128 then return 255,255,255 else return 0,0,0 end
end
giAutoGetColor=function() if gi then return autoGetColor() else return 0,0,0 end end
...
local function openGUIExample()
	local function showObjectsDialog()
		...
		:addButton{
			...
			onInit=function(self) self:setWidth(self:getParent():getClientWidth()/2-1) end,
...
addCanvas=function(self,tbl)
	...
	local enabled=true
	if type(tbl.enabled)~="nil" then enabled=tbl.enabled end
	tbl2.onInit=function(self,...)
		self.alpha=tonumber(tbl.alpha) or 1
	...
	tbl2.onDraw=function(self,...)
		setAlpha2(math.max(0,math.min(1,self:getAlpha())))
		Drawing.setAlpha=function(a)
			return setAlpha2(math.max(0,math.min(1,(tonumber(a) or 1)*self:getAlpha())))
		end
...
addButton=function(self,tbl)
	...
	return self:addCanvas {
		...
		onInit=function(self,...)
			...
			icon2=self:addIcon {
				...
				onUpdate=function(self)
					...
					local i=2
					setWidth(self,self:getParent():getWidth()-(i*2))
					setHeight(self,(self:getParent():getHeight()-(i*2)))
					setY(self,i+((self:getParent():getHeight()-(i*2))/2)-(self:getHeight()/2))
					setX(self,i+((self:getParent():getWidth()-(i*2))/2)-(self:getWidth()/2))
					if tostring(text or ""):len()>=1 then
						setWidth(self,math.min(30-(i*2),self:getWidth()))
						setX(self,math.min(15,(self:getParent():getWidth()-4)/2)-(self:getWidth()/2))
					end
				end
			}
			text2=self:addLabel {
				...
				onUpdate=function(self)
					...
					if (tonumber(icon) or 0)>=1 then
						setX(self,30+i)
						setWidth(self,self:getParent():getWidth()-30-(i*2))
						self:setAlignment(0.0,0.5)
					end
				end
			}
			...
			local iw=0
			if type(icon)=="number"then iw=Drawing.getImageSize(icon) end
			...
			local c=function()
				...
				if type(text)=="function" then pcall(function() text=text() end) end
				if type(icon)=="number"then iw=Drawing.getImageSize(icon) end
				if type(text)=="string" or type(text)=="number"then tw=Drawing.getTextSize(text) end
				if tw>0 then setWidth(self,math.max(self:getWidth(),tw+5)) end
				if type(icon)=="number" and (type(text)=="number" or type(text)=="string") and iw>0 and tw>0 then
					setWidth(self,math.max(self:getWidth(),math.min(30,iw)+tw+20))
				end
			end c()
			function self:setIcon(v) icon=v c() end
			function self:setText(v) text=v c() end
			function self:getIcon() if type(icon)=="function"then return icon()else return icon or 0 end end
		...
		onDraw=function(self,x,y,w,h)
			pcall(function()if type(tbl.clipping)=="function" then
				local x,y,w,h=tbl.clipping()
				if type(x)=="number" and type(y)=="number"and type(w)=="number"
				and type(h)=="number" then Drawing.setClipping(x,y,w,h) end
			end end)
			Drawing.setColor(giGetColor())
			Drawing.setAlpha(0.7)
			--Drawing.drawNinePatch(script:getDraft():getFrame(1),x,y,w,h)
			...
			do
				...
				Drawing.setAlpha(0)
				if self:isTouchPointInFocus() or self:getTouchPoint() or self:isMouseOver() then
					Drawing.setAlpha(0.1)
					if self:isEnabled() then Drawing.setAlpha(0.3) end
					if (self:isTouchPointInFocus() and self:isEnabled()) then Drawing.setAlpha(1) end
				end
				--Drawing.drawNinePatch(script:getDraft():getFrame(1),x,y,w,h)
				Drawing.drawRect(x,y,w,h)
				...
				if (self:isTouchPointInFocus() and self:isEnabled()) then Drawing.setColor(giGetColor()) end
				drawOutline(x,y,w,h)
				--Drawing.drawNinePatch(script:getDraft():getFrame(10),x,y,w,h)
				Drawing.setAlpha(1)
				if isPressed()then drawOutline(x+2,y+2,w-4,h-4)end
				--if isPressed()then Drawing.drawNinePatch(script:getDraft():getFrame(10),x+2,y+2,w-4,h-4) end
				Drawing.setColor(r,g,b)
			end
			Drawing.resetClipping()
			Drawing.reset()
...
addIcon=function(self,tbl)
	...
	return self:addCanvas {
		...
		onInit=function(self,...)
			...
			if type(tbl.onInit)=="function" then tbl.onInit() end
		end,
...
addTextFrame=function(self,tbl)
	...
	return self:addCanvas {
		...
		onUpdate=function(self,...)
			if self:getTouchPoint()then speedY=select(6,self:getTouchPoint()) else speedY=speedY*0.8 end
...
addTextField=function(self,tbl)
	local text
	if type(tbl.text)~="nil" then text=tbl.text end
	local ax,ay=0,0.5
	...
	local tf tf=self:addCanvas {
		...
		onInit=function(self,...)
			...
			function self:getText() return text end
			function self:setText(v) text=v pcall(function() tfe:getChild(1):setText(v) end) end
			function self:getAlignment(x,y) return ax,ay end
			function self:setAlignment(x,y) ax,ay=x,y end
		...
		onDraw=function(self,x,y,w,h,...)
			...
			Drawing.setClipping(x,y+1,w,h-1)
			drawOutline(x,y,w,h)
			Drawing.resetClipping()
...
addSlider=function(self,tbl)
	local minValue=tonumber(tbl.minValue) or 0
	local maxValue=tonumber(tbl.maxValue) or 1
	local v,bt,pb=0
	local setValue
	return self:addCanvas {
		id=tbl.id,
			...
			bt=self:addButton{
				...
				onClick=function(_,x) if x==fx2 then
					...
					GUI.createRenameDialog {
						...
						w=200,
						h=100,
						onOk=function(vv) v=math.max(minValue,math.min(maxValue,tonumber(tostring(vv or ""):trim()) or 0)) if type(tbl.setValue)=="function" then tbl.setValue(v) end end,
				...
				onUpdate=function(self)
					...
					if type(tbl.getValue)=="function" then pcall(function() v2=tbl.getValue() end) end
					v2=tonumber(v2) or 0
					v=(v2-minValue)/(maxValue-minValue)
...
createMenu=function(tbl)
	...
	GUI.getRoot():addCanvas {
		onInit=function(self)
			...
			self:setSize(GUI.getRoot():getSize())
			self:setPosition(-p2,-p3)
			local loadMenuItems loadMenuItems=function(source,actions) local pnl2="" return self:addPanel {
				w=(tbl.width or tbl.w or 120)+4,
				onInit=function(self)
					pcall(function() if type(source)=="table" then self:setPosition(source:getAbsoluteX(),source:getAbsoluteY()+source:getHeight()) end end)
							...
							onUpdate=function(self)
								...
								if (self:getTouchPoint() or self:isMouseOver()) then
									...
									if type(v.actions)=="table" and not GUI.get(pnl) then
										local pnl3=loadMenuItems(self,v.actions)
										pnl3:setId(pnl) pnl2=pnl
										pnl3:setPosition(parent:getX()+parent:getWidth(),self:getAbsY())
									end
								end
							end,
							onDraw=function(self,x,y,w,h)
				...
				onUpdate=function(self)
					self:setX(math.max(p2,math.min(self:getParent():getWidth()-p2-p4-self:getWidth(),self:getX())))
					self:setY(math.max(p3,math.min(self:getParent():getHeight()-p3-p5-self:getHeight(),self:getY())))
				end
			} end
	...
	tbl3.onInit=function(self,...)
		...
		local parent2=parent:addPanel {
			...
			onInit=function(self)
				...
				tbl2.title=self:addCanvas {
					h=30,
					onInit=function(self)
						function self:getText() return title end
						function self:setText(v) title=v end
					end,
					onDraw=function(self,x,y,w,h)
						local title=tostring(title or "")
						Drawing.setColor(autoGetColor())
						local tw,th=Drawing.getTextSize(title,Font.BIG)
						Drawing.drawText(title,x+(w/2)-(tw/2),y+(h/2)-(th/2),Font.BIG)
						Drawing.reset()
					end
				}
				self:addCanvas {
				...
				tbl2.controls=self:addLayout {
					...
					onInit=function(self)
						...
						local function addButton(v)
							...
							if type(v.autoClose)~="nil" then autoClose=v.autoClose end
							return self:getLastPart():addButton {
								w=1,
								icon=tonumber(v.icon) or 0,
								id=v.id,
								text=v.text or "",
								onInit=v.onInit,
								onUpdate=function(self,...)
									self:setPosition(self:getX(),self:getY())
									if type(v.onUpdate)=="function"then v.onUpdate(self,...); end;
								end,
								onClick=function(_,...)
				...
				self:addCanvas {
					...
					onInit=function(self)
						self:addButton {
							...
							onClick=function()
								local i=table.find(s,v:getId())
								if i then if #s>minS then table.remove(s,i) end else
									table.insert(s,v:getId()) while #s>maxS do table.remove(s,1) end
								end
							end
						}
					end
...
drawNinePatch=function(f,x,y,w,h,c)
	f=tonumber(f) or 2
	local topLeft=f
	local topCenter=f+1
	local topRight=f+2
	...
	local center=f+4
	local right=f+5
	local bottomLeft=f+6
	local bottomCenter=f+7
	local bottomRight=f+8
	local iw,ih=--{
...
function script:enterStage(s)
	...
	if type(setAlpha2)~="function" then setAlpha2=Drawing.setAlpha end
	...
	GUI.setX=function(self,x) return self:setPosition(x,self:getY()) end
	GUI.setY=function(self,y) return self:setPosition(self:getX(),y) end
	...
	GUI.getAlpha=function(self) return math.max(0,math.min(1,(tonumber(self.alpha) or 1)*settings.alpha)) end
	GUI.setAlpha=function(self,a) self.alpha=tonumber(a) or 1 end
	GUI.getHotkeys=function(self)
		local hotkeys={}
	...
	GUI.getObjects=function(self) if type(self)=="table" then
		local tbl={}
	...
	GUI.isTouchPointInFocus=function(self)
		local x,y,w,h=GUI.getAbsoluteX(self),GUI.getAbsoluteY(self),GUI.getWidth(self),GUI.getHeight(self)
...
function script:overlay()
	...
	--pcall(function() TheoTown.SETTINGS.premium=true end)
	--TheoTown.SETTINGS.premium=true
	--TheoTownTheoTown.SETTINGS.displayResolution=1000
	--error()
